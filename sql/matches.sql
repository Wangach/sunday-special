...
ALTER TABLE `sundb`. `looserdata` ADD `coup` VARCHAR(100) NOT NULL AFTER `matchty`;
...
/*Transactions table*/
CREATE TABLE `sundb`. `transactions` ( `id` INT NOT NULL AUTO_INCREMENT , `trName` VARCHAR(100) NOT NULL , `trId` VARCHAR(100) NOT NULL , `credit` INT NOT NULL , `debit` INT NOT NULL , `amount` INT NOT NULL , `pmeth` VARCHAR(100) NOT NULL , `trDesc` VARCHAR(100) NOT NULL , `trDte` TIMESTAMP on update CURRENT_TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP , PRIMARY KEY (`id`)) ENGINE = InnoDB;

/*Users Table*/
CREATE TABLE `sundb`. `users` ( `id` INT NOT NULL AUTO_INCREMENT , `username` VARCHAR(100) NOT NULL , `password` VARCHAR(100) NOT NULL , `phone` INT NOT NULL , `alias` VARCHAR(100) NOT NULL , `favteam` VARCHAR(100) NOT NULL , `uno` INT NOT NULL , `dte_registered` VARCHAR(100) NOT NULL , `last_login` TIMESTAMP on update CURRENT_TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP , `is_logged` INT NOT NULL DEFAULT '0' , `is_deleted` INT NOT NULL DEFAULT '0' , `reg_no` INT NOT NULL , PRIMARY KEY (`id`)) ENGINE = InnoDB;
...
/*Foreign Key*/
ALTER TABLE `sundb`. `admin` ADD CONSTRAINT `fk_user_category` FOREIGN KEY (`role`) REFERENCES `roles`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

/*coup and timestamp on looser pay table*/
ALTER TABLE `sundb`. `looserdata` ADD `coup` VARCHAR(100) NOT NULL AFTER `matchty`;
ALTER TABLE `sundb`. `looserdata` ADD `tme` TIMESTAMP on update CURRENT_TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP AFTER `matchid`;

/*is_paid  on fair pay table*/
ALTER TABLE `sundb`. `fairdata` ADD `is_paid` INT NOT NULL DEFAULT '0' AFTER `matchid`;
